...
// Find and remove the old lakare entry at index 12
$removed = false;
foreach ($data as $index => $entry) {
    if (isset($entry['slug']) && $entry['slug'] === 'lakare' && !isset($entry['author'])) {
        echo "Removing old lakare entry at index $index (avg_salary: {$entry['avg_salary']})\n";
        unset($data[$index]);
        $removed = true;
        break;
    }
